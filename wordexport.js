(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.wordexport) {
      window.wwload.wordexport = "wait";
      requirejs.config({
        paths: {
          "filesaver": "libs/wordexport/FileSaver", // 示例: libs/qrcodejs/qrcode
          "wordexport": "libs/wordexport/jquery.wordexport"
        }

      });
      require(["filesaver"], function () {
        require(["wordexport"], function () {
          window.wwload.wordexport = true;
          replace();
          fncallback();
        })
      });

    } else if (window.wwload.wordexport === "wait") {
      setTimeout(function () {
  ...
  function word_export($ele) {
    var config = {};
    config.elementname = $ele.attr("data-elementname");   //设置生成word文档名字
    $(".export_btn").click(function (event) {
      $ele.find(".word_Export").wordExport(config.elementname);
    });
